  ...
  return (
    <Layout>
      <div className="h-[calc(100vh-4rem)] md:h-[calc(100vh-3rem)] flex flex-col mb-20 md:mb-0">
        ...
        <section className="flex-1 bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 overflow-hidden flex flex-col" aria-label="AI chat conversation">
          <div className="flex-1 overflow-y-auto space-y-4 mb-4" role="log" aria-live="polite">
            {messages.map((msg, idx) => (
              ...
              >
                ...
                >
                  {msg.role === 'user' ? (
                    <User size={20} className="text-green-600 dark:text-green-400" aria-hidden="true" />
                  ) : (
                    <Bot size={20} className="text-blue-600 dark:text-blue-400" aria-hidden="true" />
                  )}
                </div>
                <div
            ...
            {loading && (
              <div className="flex gap-3" aria-label="AI is typing">
                <div className="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center" aria-hidden="true">
                  <Bot size={20} className="text-blue-600 dark:text-blue-400" aria-hidden="true" />
                </div>
                <div className="p-4 bg-gray-100 dark:bg-gray-700 rounded-2xl rounded-tl-md">
                  <div className="flex gap-1">
                    ...
                    <div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }} aria-hidden="true" />
                    <div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }} aria-hidden="true" />
                  </div>
                </div>
              </div>
            )}
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-3" aria-label="Send message to AI">
